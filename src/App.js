  ...
  return (
    <div class="w-full h-screen bg-gray-900">
      {uploadStatus && <div className="w-screen h-screen bg-opacity-90 fixed bg-gray-900 z-20 flex flex-col justify-center items-center">
        {uploadStatus === 'flashing' && <span class="loader"></span>}
        {uploadStatus === 'done' && <span class><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-14 h-14 text-green-400">
        ...
        {updateUploadStatusMsg && <button className="" onClick={clearStatus}>Close</button>}
      </div>}
      <form id="uploadForm" onSubmit={handleSubmit} class="flex flex-col w-4/5 sm:md-2/3 md:w-1/3 lg:w-1/4 m-auto h-full justify-center items-center gap-4">
